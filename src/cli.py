...
"""
    ...
    def run_task(self, args) -> int:
        ...
        try:
            ...
            from src.orchestrator.main_orchestrator import OrchestratorConfig
            config = OrchestratorConfig()
            if args.model:
                config.smart_model = args.model

            if args.no_tests:
                config.enable_testing = False

            # Инициализация оркестратора
            orchestrator = LocalSwarmOrchestrator(config=config)

            # Собираем контекст проекта синхронно
            project_context = f"Project: {project_path.name}\nPath: {project_path}"

            # Запуск (синхронный метод)
            result = orchestrator.run(task_description, project_context)
            ...
            if success:
                ...
                print(f"❌ ЗАДАЧА НЕ ВЫПОЛНЕНА (статус: {result.get('status')})")
            print("=" * 50)

            if result.get("successful"):
                print(f"\nУспешно: {result['successful']}/{result.get('total_tasks', 0)} задач")

            # Автокоммит (TODO: реализовать синхронную версию)
            if args.auto_commit and success:
                print("\n📦 Автокоммит отключён в текущей версии")
    ...
    async def main(self) -> int:
        ...
        if args.command == "run":
            return self.run_task(args)  # Синхронный вызов
